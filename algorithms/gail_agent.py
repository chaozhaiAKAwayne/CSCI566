...
class GAILAgent(BaseAgent):
    def __init__(self, config, ob_space, ac_space, env_ob_space , cnn_layers=1):
        ...
        if cnn_layers > 0:
            self._cnn = nn.Sequential(
                nn.Conv2d(in_channels=ob_space.shape[0], out_channels=32, kernel_size=8, stride=4),
                nn.ReLU(),
            ...
            cnn_out_size = self._cnn(torch.zeros(1, *ob_space.shape)).flatten().size(0)
            self._fc0 = nn.Linear(cnn_out_size, 256)
            self._fc1 = nn.Linear(256, ac_space.shape[0])
        else:
            self._fc0 = nn.Linear(ob_space.shape[0], 256)
            self._fc1 = nn.Linear(256, ac_space.shape[0])
